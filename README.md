...
`--config` を省略した場合、カレントディレクトリの `config/default.toml` が存在すれば自動的に読み込まれます。ファイルが存在しない場合は組み込みデフォルト値が使用されます。

> **注意**: 自動読み込みはカレントディレクトリが**リポジトリルート**であることを前提としています。別のディレクトリから実行した場合は `config/default.toml` が見つからず、組み込みデフォルトが適用されます。
